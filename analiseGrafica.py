...
# Mapeamentos do questionário socioeconômico (padrão INEP/ENCCEJA)
# Q01: Escolaridade da mãe
mapeamento_q01 = {
    'A': 'Nunca estudou',
    'B': 'Fund. Incompleto',
    'C': 'Fund. Completo',
...
# Q06: Tipo de escola cursada no Ensino Médio
mapeamento_q06 = {
    'A': 'Escola Pública',
    'B': 'Escola Privada',
    'C': 'Pública e Privada',
...
# ==========================================
# GRÁFICO 5: EVOLUÇÃO TEMPORAL DAS NOTAS (2022–2024)
# ==========================================
print("Processando Gráfico 5: Evolução Temporal das Notas...")

# Usa a base bruta para ter todos os anos; filtra apenas quem tem pelo menos uma nota
df_temporal = df_bruto.dropna(subset=list(colunas_notas.keys()), how='all').copy()

df_medias_ano = df_temporal.groupby('NU_ANO')[list(colunas_notas.keys())].mean().reset_index()
df_long_ano = pd.melt(
    df_medias_ano,
    id_vars='NU_ANO',
...
fig.suptitle('Evolução Temporal do Encceja RS (2022–2024)', fontsize=15, fontweight='bold', y=1.02)

# Painel esquerdo: notas das provas objetivas (escala TRI ~100–200)
...
sns.lineplot(
    ax=axes[0],
    data=df_tri,
    x='NU_ANO', y='Nota Média', hue='Prova',
...
for line in axes[0].lines:
    ...
    for x, y in zip(xdata, ydata):
        axes[0].annotate(f'{y:.1f}', xy=(x, y), xytext=(0, 8),
                         textcoords='offset points', ha='center', fontsize=9)

# Painel direito: Redação (escala 0–10)
df_redacao = df_long_ano[df_long_ano['Prova'] == 'Redação']

sns.lineplot(
    ax=axes[1],
    data=df_redacao,
    x='NU_ANO', y='Nota Média',
    marker='o', linewidth=2.5, color='#e74c3c', label='Redação'
)
...
for line in axes[1].lines:
    xdata, ydata = line.get_xdata(), line.get_ydata()
    for x, y in zip(xdata, ydata):
        axes[1].annotate(f'{y:.2f}', xy=(x, y), xytext=(0, 8),
...
# Gráfico complementar: taxa de participação por ano
...
for ano in [2022, 2023, 2024]:
    ...
    for col in cols_presenca:
        ...
        taxas.append({'Ano': ano, 'Prova': nomes_presenca[col], 'Taxa de Presença (%)': round(taxa, 1)})

df_taxas = pd.DataFrame(taxas)

plt.figure(figsize=(11, 5))
grafico_pres = sns.barplot(
    data=df_taxas, x='Prova', y='Taxa de Presença (%)', hue='Ano',
    palette='Blues_d'
)
plt.title('Taxa de Participação por Área e Ano (Encceja RS)', fontsize=14, fontweight='bold', pad=15)
...
# ==========================================
# GRÁFICO 6: PERFIL SOCIOECONÔMICO x DESEMPENHO
# ==========================================
print("Processando Gráfico 6: Perfil Socioeconômico x Desempenho...")

# Usa a base limpa (presentes com nota) para esta análise
df_socio = df_presentes.copy()

fig, axes = plt.subplots(1, 3, figsize=(20, 6))
fig.suptitle('Perfil Socioeconômico x Desempenho no Encceja RS', fontsize=15, fontweight='bold', y=1.02)

# --- Painel 1: Renda Familiar (Q05) x Nota Média Geral ---
df_socio['Q05_LABEL'] = df_socio['Q05'].astype(str).str.strip().map(mapeamento_q05)
df_socio['NOTA_MEDIA_GERAL'] = df_socio[['NU_NOTA_LC', 'NU_NOTA_CH', 'NU_NOTA_MT', 'NU_NOTA_CN']].mean(axis=1)
...
# --- Painel 2: Escolaridade da Mãe (Q01) x Nota Média por Área ---
df_socio['Q01_LABEL'] = df_socio['Q01'].astype(str).str.strip().map(mapeamento_q01)

# Usa apenas provas objetivas para esta comparação
...
sns.lineplot(
    ax=axes[1],
    data=df_long_escol,
    x='Escolaridade da Mãe', y='Nota Média',
    hue='Área', marker='o', linewidth=2
)
